...
1. **Low-Latency Small Payloads**: For message sizes $\le 64\text{ KiB}$ segment ($\le 256\text{ KiB}$ tensor), the Eager protocol achieves **$2.1\times\text{--}2.8\times$ lower latency** than Rendezvous ($43.3\,\mu\text{s}$ at 64 B vs $93.4\,\mu\text{s}$, down to $15.7\,\mu\text{s}$ in barrier-free microbenchmarks) by eliminating the 4-way RTS/CTS control handshake and pushing payload directly into pre-posted receive buffers.
