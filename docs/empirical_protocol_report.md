...
1. **Low-Latency Small Payloads**: For message sizes $\le 64\text{ KiB}$ segment ($\le 256\text{ KiB}$ tensor), the Eager protocol achieves **$2.8\times$ lower latency** than Rendezvous (**$15.7\,\mu\text{s}$** at 64 B vs $92.0\,\mu\text{s}$, **$16.2\,\mu\text{s}$** at 256 B) by eliminating both the 4-way RTS/CTS control handshake and the intermediate distributed barrier.
